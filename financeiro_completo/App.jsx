...
function ContasPagar({ categorias, subcategorias, empresaId, userId, onRefresh, membro, navFiltro, onNavFiltroUsado, projetos, contas: contasBancarias }) {
  ...
  const GRUPOS_DESPESA = ["despesa_operacional","despesa_financeira","imposto","taxa_bancaria","repasse_terceiros"];
  const catsDesp = (categorias||[]).filter(c=>GRUPOS_DESPESA.includes(c.grupo));
...
function Dashboard({ lancamentos, contas, categorias, subcategorias, clientes, projetos, setTela, contasReceber, contasPagar, setNavFiltro }) {
  ...
  return (
    <div>
      ...
      {(()=>{
        const cr = (contasReceber||[]).filter(c=>c.status!=="recebido"&&c.status!=="cancelado"&&c.vencimento&&(!inicio||c.vencimento>=inicio)&&(!fim||c.vencimento<=fim));
        const cp = (contasPagar||[]).filter(c=>c.status!=="pago"&&c.status!=="cancelado"&&c.vencimento&&(!inicio||c.vencimento>=inicio)&&(!fim||c.vencimento<=fim));
...
export default function App() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const [lancamentos, contas, categorias, contasReceber, subcategorias, clientes, fornecedores, projetos, contasPagar] = await Promise.all([
