...
const inputStyle = { width: "100%", background: "var(--border)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 12px", color: "var(--text)", fontSize: 13, outline: "none", boxSizing: "border-box" };
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ width: 400, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 20, padding: "48px 40px" }}>
        ...
        {[["E-mail","email"],["Senha","password"]].map(([label,type],i) => (
          <div key={label} style={{ marginBottom: 14 }}>
            ...
            <input type={type} value={i===0?email:senha} onChange={e=>(i===0?setEmail:setSenha)(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()}
              style={{ width:"100%", background:"var(--border)", border:"1px solid var(--border)", borderRadius:10, padding:"11px 14px", color:"var(--text)", fontSize:14, outline:"none", boxSizing:"border-box" }} />
          </div>
        ))}
...
function EmpresaSetup({ user, onEmpresa }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans', sans-serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet" />
      <div style={{ width:440, background:"var(--card)", border:"1px solid var(--border)", borderRadius:20, padding:"40px" }}>
        ...
        <div style={{ fontSize:13, color:"var(--sub)", marginBottom:24 }}>Logado como {user.email}</div>

        {erro && <div style={{ background:"rgba(248,113,113,0.15)", border:"1px solid rgba(248,113,113,0.3)", borderRadius:8, padding:"10px 14px", fontSize:13, color:"#f87171", marginBottom:16 }}>{erro}</div>}

        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:11, color:"var(--sub)", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:"0.07em" }}>Nome da empresa</label>
...
function Modal({ titulo, onClose, children }) {
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:18, padding:32, width:500, maxHeight:"90vh", overflowY:"auto" }}>
...
function Campo({ label, children }) {
  return (
    <div style={{ marginBottom:14 }}>
      <label style={{ fontSize:11, color:"var(--sub)", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:"0.07em" }}>{label}</label>
      {children}
    </div>
  );
}
...
function BtnRow({ onCancel, onSave, loading }) {
  return (
    <div style={{ display:"flex", gap:10, marginTop:20 }}>
      <button onClick={onCancel} style={{ flex:1, padding:"11px", borderRadius:8, border:"1px solid var(--border)", background:"transparent", color:"var(--sub)", cursor:"pointer", fontSize:13 }}>Cancelar</button>
...
function Sidebar({ tela, setTela, user, empresa, membro, onLogout }) {
  ...
  return (
    <div style={{ width:230, background:"var(--card)", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", flexShrink:0, position:"sticky", top:0, height:"100vh" }}>
      <div style={{ padding:"18px 20px 14px", borderBottom:"1px solid var(--border)" }}>
        <img src="/logo.png" alt="PRT" style={{ height:40, display:"block", marginBottom:8 }} />
        <div style={{ fontSize:11, color:"var(--sub)", fontWeight:500 }}>{empresa?.nome}</div>
        <div style={{ fontSize:10, color:"var(--muted)", marginTop:2 }}>{user?.email}</div>
        <div style={{ fontSize:10, color:"var(--muted)", marginTop:1, textTransform:"capitalize" }}>Perfil: {membro?.perfil}</div>
      </div>
      ...
      <div style={{ padding:"12px 10px", borderTop:"1px solid var(--border)" }}>
        <button onClick={onLogout} style={{ width:"100%", display:"flex", alignItems:"center", gap:9, padding:"9px 11px", borderRadius:8, border:"none", background:"transparent", color:"var(--muted)", fontSize:13, cursor:"pointer" }}>🚪 Sair</button>
      </div>
    </div>
  );
}
...
function PreviewModal({ preview, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:16, padding:24, width:"80vw", maxWidth:800, maxHeight:"90vh", display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button onClick={onClose} style={{ background:"rgba(255,255,255,0.1)", border:"none", borderRadius:7, padding:"7px 12px", color:"var(--text)", fontSize:12, cursor:"pointer" }}>✕ Fechar</button>
          </div>
        </div>
...
function Dashboard({ lancamentos, contas, investimentos, categorias }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:12, color:"var(--muted)", marginBottom:22 }}>Visão geral da empresa</div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        {[["Saldo em contas",saldo,"#fff"],["Receitas",rec,"#34d399"],["Despesas",desp,"#f87171"],["Investimentos",invest,"#a78bfa"]].map(([label,value,cor])=>(
          <div key={label} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"16px 18px" }}>
            <div style={{ fontSize:11, color:"var(--sub)", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:8 }}>{label}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:18 }}>
          <div style={{ fontSize:13, fontWeight:500, color:"var(--sub)", marginBottom:14 }}>Despesas por categoria</div>
          {porCat.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhum lançamento ainda.</div>}
          {porCat.map(cat=>(
            <div key={cat.id} style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                <span style={{ fontSize:12, color:"var(--sub)" }}>{cat.nome}</span>
                <span style={{ fontSize:12, color:"var(--sub)" }}>{fmt(cat.total)}</span>
              </div>
        ...
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:18 }}>
          <div style={{ fontSize:13, fontWeight:500, color:"var(--sub)", marginBottom:14 }}>Últimos lançamentos</div>
          {ultimos.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhum lançamento ainda.</div>}
          {ultimos.map(l=>{
            const cat=categorias.find(c=>c.id===l.categoria_id);
            return (
              <div key={l.id} style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:10, marginBottom:10, borderBottom:"1px solid var(--border)" }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:"var(--muted)" }}>{cat?.nome||"—"} · {l.data}</div>
                </div>
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"var(--muted)" }}>{lista.length} registros</div>
        </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, background:"var(--card)", border:"1px solid var(--border)", borderRadius:10, padding:"12px 16px", marginBottom:14, flexWrap:"wrap" }}>
        {[["De",dataInicio,setDataInicio],["Até",dataFim,setDataFim]].map(([label,val,set])=>(
          <div key={label} style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <input type="date" value={val} onChange={e=>set(e.target.value)} style={{ background:"var(--border)", border:"1px solid var(--border)", borderRadius:6, padding:"6px 10px", color:"var(--text)", fontSize:12, outline:"none" }} />
          </div>
        ))}
        ...
        <button onClick={()=>{setDataInicio("");setDataFim("");}} style={{ background:"var(--border)", border:"1px solid var(--border)", borderRadius:6, padding:"6px 12px", color:"var(--sub)", fontSize:12, cursor:"pointer" }}>Tudo</button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:18 }}>
        {[["Total de receitas",totalGeral.receita,"#34d399"],["Total de despesas",totalGeral.despesa,"#f87171"],["Resultado",totalGeral.receita-totalGeral.despesa,totalGeral.receita-totalGeral.despesa>=0?"#34d399":"#f87171"]].map(([l,v,c])=>(
          <div key={l} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:10, padding:"12px 16px" }}>
            <div style={{ fontSize:10, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:5 }}>{l}</div>
            <div style={{ fontSize:17, fontWeight:600, color:c }}>{fmt(v)}</div>
          </div>
        ))}
      </div>

      {porDia.length===0 && <div style={{ fontSize:13, color:"var(--muted)", padding:20 }}>Nenhum lançamento encontrado.</div>}
      {porDia.map(([data,itens])=>{
        ...
        return (
          <div key={data} style={{ marginBottom:8 }}>
            <div onClick={()=>toggleDia(data)} style={{ display:"flex", alignItems:"center", gap:14, background:"var(--card)", border:"1px solid var(--border)", borderRadius:aberto?"10px 10px 0 0":10, padding:"12px 16px", cursor:"pointer", userSelect:"none" }}>
              <div style={{ textAlign:"center", minWidth:44, background:"rgba(99,102,241,0.15)", borderRadius:8, padding:"6px 8px" }}>
                ...
                <div style={{ fontSize:10, color:"var(--sub)", textTransform:"uppercase" }}>{mes}</div>
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, color:"var(--text)", fontWeight:500 }}>{semana}</div>
                <div style={{ fontSize:11, color:"var(--muted)" }}>{itens.length} lançamento{itens.length!==1?"s":""}</div>
              </div>
              <div style={{ display:"flex", gap:20, alignItems:"center" }}>
                {rec>0 && <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:10, color:"var(--muted)", textTransform:"uppercase" }}>Entrada</div>
                  <div style={{ fontSize:14, fontWeight:600, color:"#34d399" }}>{fmt(rec)}</div>
                </div>}
                {desp>0 && <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:10, color:"var(--muted)", textTransform:"uppercase" }}>Saída</div>
                  <div style={{ fontSize:14, fontWeight:600, color:"#f87171" }}>{fmt(desp)}</div>
                </div>}
                <div style={{ fontSize:16, color:"var(--muted)" }}>{aberto?"▲":"▼"}</div>
              </div>
            </div>
            {aberto && (
              <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderTop:"none", borderRadius:"0 0 10px 10px", overflow:"hidden" }}>
                {itens.map((l,i)=>{
                  const cat=categorias.find(c=>c.id===l.categoria_id);
                    ...
                    <div key={l.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 16px", borderBottom:i<itens.length-1?"1px solid rgba(255,255,255,0.04)":"none" }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontSize:11, color:"var(--muted)", display:"flex", gap:6, marginTop:2, flexWrap:"wrap" }}>
                          ...
                          {sub && <span style={{ color:"var(--sub)" }}>› {sub.nome}</span>}
                      ...
                      <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
                        ...
                        {podeExcluir && <button onClick={()=>excluir(l.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}
                      </div>
                    </div>
                  );
                })}
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"var(--muted)" }}>{contas.length} contas cadastradas</div>
        </div>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"14px 20px", marginBottom:16, display:"flex", gap:28, flexWrap:"wrap", alignItems:"center" }}>
        <div style={{ fontSize:12, fontWeight:600, color:"var(--sub)", textTransform:"uppercase", letterSpacing:"0.06em" }}>Totais em aberto</div>
        {[["Custo Fixo",totais.fixo,"#818cf8"],["Custo Variável",totais.variavel,"#fbbf24"],["Investimento",totais.investimento,"#34d399"],["Total",totalGeral,"#f87171"]].map(([l,v,c])=>(
          <div key={l}><div style={{ fontSize:10, color:"var(--muted)", textTransform:"uppercase", marginBottom:3 }}>{l}</div><div style={{ fontSize:15, fontWeight:600, color:c }}>{fmt(v)}</div></div>
        ))}
      </div>
      ...
      {!carregando && diasOrdenados.length===0 && <div style={{ color:"var(--muted)", fontSize:13, padding:20 }}>Nenhuma conta encontrada.</div>}
          ...
          <div key={data} style={{ marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:14, background:"var(--card)", border:`1px solid ${corDia}44`, borderRadius:"10px 10px 0 0", padding:"10px 16px" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:11, color:"var(--muted)" }}>{itens.length} conta{itens.length!==1?"s":""}</div>
              </div>
            ...
            {itens.map((c,i)=>{
              ...
              return (
                <div key={c.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderTop:"none", borderRadius:i===itens.length-1?"0 0 10px 10px":0, padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  {podeExcluir && <button onClick={()=>excluir(c.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}
                </div>
              );
            })}
          </div>
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
        <div><div style={{ fontSize:22, fontWeight:600, color:"var(--text)", marginBottom:4 }}>Contas bancárias</div><div style={{ fontSize:12, color:"var(--muted)" }}>Saldo consolidado: {fmt(total)}</div></div>
        {podeCriar && <button onClick={()=>setModal(true)} style={{ background:"#6366f1", border:"none", borderRadius:8, padding:"9px 16px", color:"#fff", fontSize:13, fontWeight:500, cursor:"pointer" }}>+ Nova conta</button>}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
        {contas.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhuma conta cadastrada.</div>}
        {contas.map(c=>(
          <div key={c.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:18, position:"relative" }}>
            {podeExcluir && <button onClick={()=>excluir(c.id)} style={{ position:"absolute", top:12, right:12, background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}
            ...
            <div style={{ fontSize:12, color:"var(--sub)", marginBottom:4 }}>{c.banco}</div>
            ...
            <div style={{ fontSize:11, color:"var(--muted)", marginTop:4 }}>{total?((Number(c.saldo)/total)*100).toFixed(1):0}% do total</div>
          </div>
        ))}
      </div>
      {modal && (
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:18 }}>
        {[["Valor atual",fmt(tAtual),"#fff"],["Rendimento",fmt(rend),"#34d399"],["Rentabilidade",`+${pct}%`,"#a78bfa"]].map(([l,v,c])=>(
          <div key={l} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"16px 18px" }}>
            <div style={{ fontSize:11, color:"var(--sub)", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:8 }}>{l}</div>
            <div style={{ fontSize:20, fontWeight:600, color:c }}>{v}</div>
          </div>
        ))}
      </div>
      {investimentos.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhum investimento.</div>}
      {investimentos.map(inv=>{
        ...
        return (
          <div key={inv.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"14px 16px", marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:16 }}>
              ...
              {podeExcluir && <button onClick={()=>excluir(inv.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}
            </div>
          </div>
        );
      })}
...
function Orcamento({ orcamento, lancamentos, categorias, empresaId, onRefresh, membro }) {
  ...
  return (
    <div>
      ...
      {orcamento.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhum orçamento definido.</div>}
      {orcamento.map(o=>{
        ...
        return (
          <div key={o.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"16px 18px", marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <span style={{ fontSize:11, color:"var(--muted)" }}>{o.mes}/{o.ano}</span>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                <span style={{ fontSize:12, color:"var(--sub)" }}>{fmt(gasto)} de {fmt(o.limite)}</span>
                <span style={{ fontSize:12, fontWeight:500, color:cor }}>{pct.toFixed(0)}%</span>
                {pct>=90 && <span>⚠️</span>}
                <button onClick={()=>excluir(o.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>
              </div>
            </div>
            <div style={{ height:5, background:"var(--border)", borderRadius:999 }}>
              <div style={{ width:`${pct}%`, height:"100%", borderRadius:999, background:cor }} />
            </div>
            <div style={{ fontSize:11, color:"var(--muted)", marginTop:4 }}>Restam: {fmt(Number(o.limite)-gasto)}</div>
          </div>
        );
      })}
      {modal && (
...
function Categorias({ categorias, empresaId, onRefresh, membro }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
        <div><div style={{ fontSize:22, fontWeight:600, color:"var(--text)", marginBottom:4 }}>Categorias</div><div style={{ fontSize:12, color:"var(--muted)" }}>{categorias.length} categorias · {subcats.length} subcategorias</div></div>
      ...
      {categorias.length===0 && <div style={{ fontSize:13, color:"var(--muted)" }}>Nenhuma categoria. Crie a primeira!</div>}
      {categorias.map(cat=>{
        const subs=subcats.filter(s=>s.categoria_id===cat.id);
        const aberto=expandido[cat.id];
        return (
          <div key={cat.id} style={{ marginBottom:8 }}>
            <div style={{ background:"var(--card)", borderLeft:`3px solid ${cat.cor||"#6366f1"}`, border:"1px solid var(--border)", borderRadius:aberto&&subs.length>0?"10px 10px 0 0":10, padding:"12px 14px", display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, color:"var(--text)", fontWeight:500 }}>{cat.nome}</div>
                <div style={{ fontSize:11, color:"var(--muted)", textTransform:"capitalize", marginTop:2 }}>{cat.tipo} · {subs.length} subcategoria{subs.length!==1?"s":""}</div>
              </div>
              ...
              {subs.length>0 && <button onClick={()=>toggle(cat.id)} style={{ background:"var(--border)", border:"none", borderRadius:6, padding:"4px 10px", color:"var(--sub)", fontSize:11, cursor:"pointer" }}>{aberto?"▲":"▼"}</button>}
              {podeExcluir && <button onClick={()=>excluir(cat.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}
            </div>
            {aberto && subs.map((sub,i)=>(
              <div key={sub.id} style={{ background:"var(--card)", borderLeft:`3px solid ${sub.cor||cat.cor||"#6366f1"}`, border:"1px solid var(--border)", borderTop:"none", borderRadius:i===subs.length-1?"0 0 10px 10px":0, padding:"9px 14px 9px 24px", display:"flex", alignItems:"center", gap:10 }}>
                <div style={{ width:6, height:6, borderRadius:"50%", background:sub.cor||"#6366f1", flexShrink:0 }} />
                <div style={{ flex:1, fontSize:12, color:"var(--sub)" }}>{sub.nome}</div>
                {podeExcluir && <button onClick={()=>excluirSub(sub.id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:13 }}>🗑</button>}
              </div>
            ))}
          </div>
        );
...
function Relatorios({ lancamentos, categorias }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:12, color:"var(--muted)", marginBottom:18 }}>Análise financeira por período</div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, background:"var(--card)", border:"1px solid var(--border)", borderRadius:10, padding:"12px 16px", marginBottom:14, flexWrap:"wrap" }}>
        {[["De",inicio,setInicio],["Até",fim,setFim]].map(([label,val,set])=>(<div key={label} style={{ display:"flex", alignItems:"center", gap:8 }}><span style={{ fontSize:12, color:"var(--sub)" }}>{label}</span><input type="date" value={val} onChange={e=>set(e.target.value)} style={{ background:"var(--border)", border:"1px solid var(--border)", borderRadius:6, padding:"6px 10px", color:"var(--text)", fontSize:12, outline:"none" }} /></div>))}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:20 }}>
        {[["Receitas",rec,"#34d399"],["Despesas",desp,"#f87171"],["Resultado",res,res>=0?"#34d399":"#f87171"]].map(([l,v,c])=>(<div key={l} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:"16px 18px" }}><div style={{ fontSize:11, color:"var(--sub)", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:8 }}>{l}</div><div style={{ fontSize:20, fontWeight:600, color:c }}>{fmt(v)}</div></div>))}
      </div>

      {aba==="balancete" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:20 }}>
            <div style={{ fontSize:13, fontWeight:500, color:"var(--sub)", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.06em" }}>Despesas por categoria</div>
            {porCat.length===0 && <div style={{ color:"var(--muted)", fontSize:13 }}>Sem dados.</div>}
            {porCat.map(cat=>(<div key={cat.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}><div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ width:8, height:8, borderRadius:2, background:cat.cor||"#6366f1" }} /><span style={{ fontSize:13, color:"var(--sub)" }}>{cat.nome}</span></div><div style={{ textAlign:"right" }}><div style={{ fontSize:13, fontWeight:500, color:"#f87171" }}>{fmt(cat.total)}</div><div style={{ fontSize:10, color:"var(--muted)" }}>{desp?((cat.total/desp)*100).toFixed(1):0}%</div></div></div>))}
            {desp>0 && <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0 0", marginTop:4, borderTop:"1px solid var(--border)" }}><span style={{ fontSize:13, fontWeight:500, color:"var(--text)" }}>Total</span><span style={{ fontSize:13, fontWeight:600, color:"#f87171" }}>{fmt(desp)}</span></div>}
          </div>
          <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:20 }}>
            <div style={{ fontSize:13, fontWeight:500, color:"var(--sub)", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.06em" }}>Receitas por categoria</div>
            {porCatRec.length===0 && <div style={{ color:"var(--muted)", fontSize:13 }}>Sem dados.</div>}
            {porCatRec.map(cat=>(<div key={cat.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}><div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ width:8, height:8, borderRadius:2, background:cat.cor||"#6366f1" }} /><span style={{ fontSize:13, color:"var(--sub)" }}>{cat.nome}</span></div><div style={{ fontSize:13, fontWeight:500, color:"#34d399" }}>{fmt(cat.total)}</div></div>))}
          </div>
        </div>
      )}
      ...
      {aba==="caixa" && (
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)", background:"rgba(255,255,255,0.02)" }}>{["Data","Dia","Entradas","Saídas","Saldo do dia","Saldo acum."].map(h=>(<th key={h} style={{ padding:"11px 14px", textAlign:"left", fontSize:10, color:"var(--muted)", fontWeight:500, textTransform:"uppercase", letterSpacing:"0.06em" }}>{h}</th>))}</tr></thead>
            <tbody>
              {porDia.length===0 && <tr><td colSpan={6} style={{ padding:24, textAlign:"center", color:"var(--muted)", fontSize:13 }}>Sem movimentações.</td></tr>}
              {(()=>{ let acum=0; return porDia.map(([data,v])=>{ const sd=v.rec-v.desp; acum+=sd; const [y,m,d]=data.split("-"); const ds=dias[new Date(Number(y),Number(m)-1,Number(d)).getDay()]; return (<tr key={data} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}><td style={{ padding:"10px 14px", fontSize:12, color:"var(--sub)" }}>{d}/{m}/{y}</td><td style={{ padding:"10px 14px", fontSize:12, color:"var(--sub)" }}>{ds}</td><td style={{ padding:"10px 14px", fontSize:12, color:"#34d399", fontWeight:500 }}>{v.rec>0?fmt(v.rec):"—"}</td><td style={{ padding:"10px 14px", fontSize:12, color:"#f87171", fontWeight:500 }}>{v.desp>0?fmt(v.desp):"—"}</td><td style={{ padding:"10px 14px", fontSize:12, fontWeight:600, color:sd>=0?"#34d399":"#f87171" }}>{fmt(sd)}</td><td style={{ padding:"10px 14px", fontSize:12, fontWeight:600, color:acum>=0?"#818cf8":"#f87171" }}>{fmt(acum)}</td></tr>); }); })()}
            </tbody>
          </table>
        </div>
      )}
      ...
      {aba==="dre" && (
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:24 }}>
          <div style={{ fontSize:11, fontWeight:500, color:"var(--sub)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:16 }}>DRE — {inicio} a {fim}</div>
          <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:"1px solid var(--border)" }}><span style={{ fontSize:13, color:"var(--sub)" }}>( + ) Receita bruta</span><span style={{ fontSize:13, color:"#34d399", fontWeight:500 }}>{fmt(rec)}</span></div>
          {porCat.map(cat=>(<div key={cat.id} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0 9px 14px", borderBottom:"1px solid rgba(255,255,255,0.04)" }}><span style={{ fontSize:12, color:"var(--sub)" }}>( − ) {cat.nome}</span><span style={{ fontSize:12, color:"#f87171" }}>({fmt(cat.total)})</span></div>))}
          {porCat.length===0 && <div style={{ padding:"9px 0 9px 14px", fontSize:12, color:"var(--muted)" }}>Nenhuma despesa.</div>}
          <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderTop:"1px solid var(--border)", marginTop:6 }}><span style={{ fontSize:13, color:"var(--sub)" }}>( = ) Despesas totais</span><span style={{ fontSize:13, color:"#f87171", fontWeight:500 }}>({fmt(desp)})</span></div>
          <div style={{ display:"flex", justifyContent:"space-between", padding:"14px 0 4px", borderTop:"2px solid rgba(255,255,255,0.15)", marginTop:4 }}><span style={{ fontSize:15, color:"var(--text)", fontWeight:500 }}>( = ) Resultado líquido</span><div style={{ textAlign:"right" }}><div style={{ fontSize:22, fontWeight:600, color:res>=0?"#34d399":"#f87171" }}>{fmt(res)}</div><div style={{ fontSize:11, color:"var(--muted)" }}>Margem: {rec?((res/rec)*100).toFixed(1):"0.0"}%</div></div></div>
        </div>
      )}
      ...
      {aba==="categorias_rel" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          {[["Receitas por categoria",porCatRec,rec,"#34d399"],["Despesas por categoria",porCat,desp,"#f87171"]].map(([titulo,dados,total,cor])=>(
            <div key={titulo} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, padding:20 }}>
              <div style={{ fontSize:13, fontWeight:500, color:"var(--sub)", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.06em" }}>{titulo}</div>
              {dados.length===0 && <div style={{ color:"var(--muted)", fontSize:13 }}>Sem dados.</div>}
              {dados.map(cat=>(<div key={cat.id} style={{ marginBottom:12 }}><div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}><span style={{ fontSize:12, color:"var(--sub)" }}>{cat.nome}</span><span style={{ fontSize:12, color:cor }}>{fmt(cat.total)}</span></div><div style={{ height:4, background:"var(--border)", borderRadius:999 }}><div style={{ width:`${total?(cat.total/total*100):0}%`, height:"100%", background:cat.cor||cor, borderRadius:999 }} /></div></div>))}
            </div>
          ))}
        </div>
      )}
...
function Usuarios({ empresa, userId }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
        <div><div style={{ fontSize:22, fontWeight:600, color:"var(--text)", marginBottom:4 }}>Usuários</div><div style={{ fontSize:12, color:"var(--muted)" }}>{membros.length} membros da empresa</div></div>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden" }}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>{["E-mail","Nome","Perfil","Desde",""].map(h=>(<th key={h} style={{ padding:"11px 16px", textAlign:"left", fontSize:10, color:"var(--muted)", fontWeight:500, textTransform:"uppercase" }}>{h}</th>))}</tr></thead>
          <tbody>
            {membros.map(m=>(<tr key={m.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
              ...
              <td style={{ padding:"12px 16px", fontSize:13, color:"var(--sub)" }}>{m.nome||"—"}</td>
              ...
              <td style={{ padding:"12px 16px", fontSize:12, color:"var(--muted)" }}>{new Date(m.created_at).toLocaleDateString("pt-BR")}</td>
              <td style={{ padding:"12px 16px" }}>{m.user_id!==userId && <button onClick={()=>remover(m.id,m.user_id)} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", fontSize:14 }}>🗑</button>}</td>
            </tr>))}
          </tbody>
        </table>
      </div>
      ...
      {modal && (
        <Modal titulo="Convidar usuário" onClose={()=>{ setModal(false); setLinkGerado(""); setForm({email:"", perfil:"financeiro"}); }}>
          {!linkGerado ? (
            <>
              <div style={{ background:"rgba(99,102,241,0.1)", border:"1px solid rgba(99,102,241,0.2)", borderRadius:8, padding:"12px 14px", fontSize:13, color:"var(--sub)", marginBottom:16 }}>
            ...
            <>
              ...
              <div style={{ background:"var(--border)", border:"1px solid var(--border)", borderRadius:8, padding:"12px 14px", fontSize:12, color:"#818cf8", wordBreak:"break-all", marginBottom:14, fontFamily:"monospace" }}>
                {linkGerado}
              </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={()=>{ setModal(false); setLinkGerado(""); setForm({email:"", perfil:"financeiro"}); }} style={{ flex:1, padding:"11px", borderRadius:8, border:"1px solid var(--border)", background:"transparent", color:"var(--sub)", cursor:"pointer", fontSize:13 }}>Fechar</button>
...
export default function App() {
  ...
  if (verificando) return (<div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", alignItems:"center", justifyContent:"center", color:"var(--sub)", fontFamily:"'DM Sans', sans-serif", fontSize:14 }}><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet" />Verificando acesso...</div>);
  ...
  const estilos = tema === "claro" ? {
    ...
    "--input":"var(--border)", "--hover":"var(--border)", "--active-bg":"rgba(99,102,241,0.15)", "--shadow":"none",
  };
    ...
    <div style={{ display:"flex", minHeight:"100vh", fontFamily:"'DM Sans', sans-serif", background:"var(--bg)", color:"var(--text)", transition:"background 0.3s, color 0.3s", ...estilos }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        input:not([type="file"]):not([type="color"]), select, textarea {
      ...
      <div style={{ flex:1, overflowY:"auto" }}>
        <div style={{ padding:"28px 32px", maxWidth:1100 }}>
          {carregando ? (
            <div style={{ color:"var(--muted)", fontSize:14, paddingTop:40 }}>Carregando...</div>
          ) : (
            <>
