  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0a0a0f", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ width:440, background:"#13131a", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:"40px" }}>
        ...
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.4)", marginBottom:24 }}>Logado como {user.email}</div>

        {erro && <div style={{ background:"rgba(248,113,113,0.15)", border:"1px solid rgba(248,113,113,0.3)", borderRadius:8, padding:"10px 14px", fontSize:13, color:"#f87171", marginBottom:16 }}>{erro}</div>}

        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:11, color:"rgba(255,255,255,0.4)", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:"0.07em" }}>Nome da empresa</label>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, background:"#1a1a2e", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:"12px 16px", marginBottom:14, flexWrap:"wrap" }}>
        ...
        <button onClick={()=>{setDataInicio("");setDataFim("");}} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:6, padding:"6px 12px", color:"rgba(255,255,255,0.4)", fontSize:12, cursor:"pointer" }}>Tudo</button>
      </div>
        ...
        return (
          <div key={cat.id} style={{ marginBottom:8 }}>
            <div style={{ background:"#1a1a2e", borderLeft:`3px solid ${cat.cor||"#6366f1"}`, border:"1px solid rgba(255,255,255,0.07)", borderRadius:aberto&&subs.length>0?"10px 10px 0 0":10, padding:"12px 14px", display:"flex", alignItems:"center", gap:10 }}>
              ...
              {subs.length>0 && <button onClick={()=>toggle(cat.id)} style={{ background:"rgba(255,255,255,0.05)", border:"none", borderRadius:6, padding:"4px 10px", color:"rgba(255,255,255,0.4)", fontSize:11, cursor:"pointer" }}>{aberto?"▲":"▼"}</button>}
...
function Relatorios({ lancamentos, categorias }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, background:"#1a1a2e", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:"12px 16px", marginBottom:14, flexWrap:"wrap" }}>
        {[["De",inicio,setInicio],["Até",fim,setFim]].map(([label,val,set])=>(<div key={label} style={{ display:"flex", alignItems:"center", gap:8 }}><span style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>{label}</span><input type="date" value={val} onChange={e=>set(e.target.value)} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:6, padding:"6px 10px", color:"#fff", fontSize:12, outline:"none" }} /></div>))}
      ...
      {aba==="dre" && (
        <div style={{ background:"#1a1a2e", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:24 }}>
          ...
          {porCat.map(cat=>(<div key={cat.id} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0 9px 14px", borderBottom:"1px solid rgba(255,255,255,0.04)" }}><span style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>( − ) {cat.nome}</span><span style={{ fontSize:12, color:"#f87171" }}>({fmt(cat.total)})</span></div>))}
...
export default function App() {
  ...
  if (verificando) return (<div style={{ minHeight:"100vh", background:"#0a0a0f", display:"flex", alignItems:"center", justifyContent:"center", color:"rgba(255,255,255,0.4)", fontFamily:"'DM Sans', sans-serif", fontSize:14 }}><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet" />Verificando acesso...</div>);
